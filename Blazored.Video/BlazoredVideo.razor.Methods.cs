...
namespace Blazored.Video
{
	...
	{
		/// <summary>
		/// Tell the video player to enter PIP
		/// </summary>
		public ValueTask RequestPictureInPicture() => DoInvokeAsync(method: "requestPictureInPicture");
		/// <summary>
		/// Tell the video player to exit PIP
		/// </summary>
		public ValueTask ExitPictureInPicture() => DoInvokeAsync(method: "exitPictureInPicture");
		...
		{
			try {
				if (jsModule is null) return ValueTask.FromResult(default(T));
		...
		protected virtual ValueTask SetValueAsync<T>(T value, [CallerMemberName] string name = null)
		{
			try
			{
				if (jsModule is null) return ValueTask.CompletedTask;
				var payloadName = VideoStateOptionsExt.FormatGetSetAsyncAsPayload(name);
				if (jsModule is IJSInProcessObjectReference inProcessModule)
				{
